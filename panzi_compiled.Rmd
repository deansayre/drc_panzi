...
weekly_map <- function(x){

  out <- leaflet::leaflet(x) %>% 
   addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
   addCircleMarkers(radius = ~sqrt(num),
                    color = "#1e5597",
                    fill = TRUE,
                    fillOpacity = 0.5, 
                    group = "Weekly Cases") %>% 
   addCircleMarkers(radius = ~sqrt(cum_cases),
                    color = "#1e5597",
                    fill = TRUE,
                    fillOpacity = 0.5, 
                    group = "Cumulative Cases") %>% 
   addCircleMarkers(radius = ~sqrt(deaths),
                    color = "#972b1e",
                    stroke = FALSE, fillOpacity = 0.5, 
                    group = "Weekly Deaths") %>% 
   addCircleMarkers(radius = ~sqrt(cum_deaths),
                    color = "#972b1e",
                    stroke = FALSE, fillOpacity = 0.5, 
...
too_many_maps <- map(map, ~weekly_map(.x))


#  make_tab <- function(map, name) {         # function to make the tabs
#    cat("::: {.panel}\n")              # Open tab
#    cat("##", name, "{.panel-name}\n") # Label tab
#  
#    map                                # Display plot
...
#  }
#  
#  walk2(too_many_maps, names(too_many_maps), ~make_tab(.x, .y))
#    
...
ggplot(aes(fill = waffle_cat, values = count)) +
  ...
  facet_wrap(~year_week_factor, nrow = 1, strip.position = "top", drop = FALSE) +
