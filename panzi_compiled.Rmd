...
The Democratic Republic of the Congo (DRC) is highly endemic for malaria 
...
```{r}
routine_case <- rio::import(here("routine_data", "cas_confirme_02_27_25.xlsx")) %>% 
  pivot_longer(-c(1,2)) %>% 
...
tests_list <- map(unique(tests_farr$name), 
               ~tests_farr %>% 
                 filter(name == .x) %>% 
                 split(.$hf) %>% 
                 map(~.x %>% 
                 ...
                 pivot_wider(names_from = "name", 
                             values_from = "value")) %>% 
  set_names(unique(tests_farr$name)) %>% 
  reduce(~left_join(.x,.y, by = c("hf", "month_year"))) %>% 
  mutate(across(c(3:7), ~replace_na(.x, 0)), 
         total = rowSums(select(., c(3:7)), na.rm = TRUE))
