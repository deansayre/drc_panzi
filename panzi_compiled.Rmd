...
#              group = "Population density") %>% 
  ...
  addPolygons(
    ...
    addLayersControl(baseGroups = c("MAP Incidence 2021",
                       ...
                       overlayGroups = "Panzi Health Zone", 
...
routine_data_panzi_split <- routine_data_panzi %>% 
  rename(month_year = name) %>%
  mutate(across(c(5:10), ~case_when(is.na(.x) ~ 0, 
                           .default = .x))) %>% 
  split(.$hf) %>% 
  map(~.x %>% 
        group_by(year, month) %>% 
        summarise(rdt_fac = hf_rdt_5plus+hf_rdt_cu5,
                  rdt_comm = comm_cu_5_tdr+comm_5_plus_tdr, 
                  ge_pos = ge_pos, 
                  overall = rdt_fac + rdt_comm + ge_pos) %>% 
        arrange(year, month)
  )
...
graphs <- pmap(list(a_list, hf_names, missing_counts_list),
       function(x,y,z) x %>% 
        ggplot()+
...
### to delete ###
routine_data_panzi_split_metric <- routine_data_panzi %>% 
  rename(month_year = name) %>%
  mutate(across(c(5:10), ~case_when(is.na(.x) ~ 0, 
                           .default = .x))
         ) %>% 
