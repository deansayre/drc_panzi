...
color3 <- c("cadetblue4", "firebrick4", "white")
...
cols2 <- c("grippe", "bronchite", "ira", "syndrome_grippal",
           "pneumonie","broncho_pneumonie", "paludisme_1")


# number of respiratory diagnoses each participant carries
#  test <- data5_a %>% 
#    select(n, cols2) %>% 
#    pivot_longer(-n) %>% 
#    filter(value == "oui") %>% 
...
test2 <- test %>% 
  ...
  arrange(-tot) %>% 
  slice_head(n = 10) %>% 
  mutate(dx = map_chr(list_dx, paste, collapse = ", "), 
         dx = fct_reorder(dx, -tot))

sympt_dx <- left_join(select(test,-disease_dx), sympt, by = "n") %>% 
  filter(list_dx %in% test2$list_dx) %>% 
  unnest_longer(list_sx)
...
heatmap <- data5 %>% 
  ...
  ungroup() %>% 
  pivot_longer(-1) %>% 
  mutate(list_dx = factor(map_chr(list_dx, paste, collapse = ", "), 
                          levels = test2$dx), 
...
bottom <- ggplot(heatmap)+
  geom_tile(aes(x = list_dx, y = name, fill = value))
...
outcome_by_presentation <- data5 %>% 
  ...
  mutate(list_sympt = map_chr(list_sympt, paste, collapse = ", "), 
         list_sympt = str_to_title(list_sympt), 
         list_sympt = str_replace_all(list_sympt, "_", " ")) %>% 
  rename(Presentation = list_sympt, 
         Count = count, 
         `Test posititivity rate (%)` = tpr,
         `Case fatality rate (%)` = taux_mortalite)

flextable::flextable(outcome_by_presentation) %>% 
  flextable::autofit()


missing_one <- outcome_by_presentation %>% 
  rowwise() %>% 
  filter(!all(c("fievre", "toux") %in% Presentation)) 
  
missing_both <- outcome_by_presentation %>% 
    rowwise() %>% 
    filter(!any(c("fievre", "toux") %in% Presentation)) 
  
#  symptom_combos <- map(list(outcome_by_presentation, missing_one, missing_both),
#      ~.x %>% rowwise()%>% 
#        mutate(Presentation =  str_to_title(paste(Presentation, collapse = ", ")), 
#               Presentation = str_replace_all(Presentation, "_", " ")) %>% 
#        rename(presentation_clinique = Presentation, 
...
#       })
#  
#  cut from below  `r sum(symptom_combos[[3]]$nombre, na.rm = TRUE)`
#  
```


There are CUT patients that were 
...
### Presentation of individuals classified as having malaria
