...
weekly_map <- function(x, db){
  maxes <- db %>% 
    summarise(cases = max(num, na.rm = TRUE), 
  ...
  addPolygons(
    ...
    stroke = TRUE, fillOpacity = 0, smoothFactor = 0.5,
    color = "black", opacity = 1, weight = 2) %>% 
    addLayersControl(baseGroups = c("Reported Cases",
                                    "Reported Deaths"),
               options = layersControlOptions(collapsed = F))
}


leaflet::leaflet(map[["2024 W50"]]) %>% 
   addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
   addCircleMarkers(radius = ~sqrt(num),
                    color = "#1e5597",
                    fill = TRUE,
