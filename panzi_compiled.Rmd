...
tdr_over5 <- tdr_over5_a %>% 
  mutate(total_rdt_5plus = case_when(!is.na(num_1) ~ num_1, 
                           !is.na(num_2) ~ num_2,
...
tdr_under_5 <- tdr_under_5_a %>% 
  ungroup() %>% 
  group_by(hf, name) %>% 
  mutate(total = case_when(map_lgl(list(value), ~all(is.na(.x))) == TRUE ~ NA_real_, 
                           .default = sum(value, na.rm = TRUE))
         )%>% 
  distinct(hf, name, total_rdt_cu5)


tdr <- left_join(tdr_over5, tdr_under_5, by =c("hf", "name")) %>% 
...
#  Hmisc::describe(tdr_over5$num_3)  #cols 3 and 4 are blank
#  Hmisc::describe(tdr_over5$num_4)
  ...
  group_by(hf, name, dataname) %>% 
  mutate(n = n(), 
         dup_value = ifelse(n>1, map(list(value), 
                                     na.omit), NA)) %>% 
  ungroup() %>% 
  rowwise() %>% 
  mutate(length = length(dup_value))
