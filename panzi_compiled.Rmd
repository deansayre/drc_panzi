...
start_month <- as.numeric(word(glue::glue_collapse(as.character(farr@start), sep = "_"), 2, 
                    sep = "_"))
start_year <- as.numeric(word(glue::glue_collapse(as.character(farr@start), sep = "_"), 1, 
                    sep = "_"))
...
start_month <- as.numeric(word(glue::glue_collapse(as.character(farr_imput_cases@start), sep = "_"), 2, 
                    sep = "_"))
start_year <- as.numeric(word(glue::glue_collapse(as.character(farr_imput_cases@start), sep = "_"), 1, 
                    sep = "_"))
...
a_new <- as.data.frame(farr_imput_cases@upperbound) %>% 
  pivot_longer(cols = everything()) %>% 
  group_by(name) %>% 
  mutate(month = row_number()) %>% 
  ungroup() %>% 
  ...
  left_join(select(ts_list_join, hf, month, year, total), 
            by = c("name" = "hf", 
                   ...
                   "year_trans" = "year")) %>% 
  rename(threshold = value) %>% 
  mutate(outbreak = case_when(total > threshold ~ "outbreak", 
                              total <= threshold ~ "routine", 
                              is.na(threshold) ~ NA_character_, 
                              .default = "error"), 
         year_month = factor(paste0(year_trans, "_", month_trans), 
                             levels = lev), 
